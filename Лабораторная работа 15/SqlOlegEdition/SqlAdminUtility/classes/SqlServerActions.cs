...
namespace SqlAdminUtility.classes
{
    class SqlServerActions
    {
        ...
        public JobManager[] GetAgentJobs()
        {
            SqlConnection sqlConnection = new SqlConnection(ConnStringClass.connStr);
            string que = "USE msdb EXEC dbo.sp_help_job";
            sqlConnection.Open();
            List<JobManager> jobManagers = new List<JobManager>();
            SqlCommand command = new SqlCommand(que, sqlConnection);
            {
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        JobManager jobManager =
                            new JobManager(reader.GetGuid(0),
                                reader.GetString(2),
                                true,
                                reader.GetString(4),
                                reader.GetString(6));
